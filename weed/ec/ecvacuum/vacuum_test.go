...
// appendTestNeedle writes one needle into a live volume via the public
// WriteNeedleBlob path (which also updates the .idx needle map).
func appendTestNeedle(t *testing.T, v *storage.Volume, dir string, id uint64, data []byte) {
	t.Helper()
...
// EC shards, and (simulating runtime deletes) journals the first `deleted` needle
// ids to .ecj — the on-disk shape VacuumLocalDir must reclaim.
func buildEncodedFixture(t *testing.T, dir, collection string, vid uint32, live, deleted int) Options {
	t.Helper()
	...
	for i := 1; i <= live; i++ {
		appendTestNeedle(t, v, dir, uint64(i), data)
	}
	v.Close()
...
// TestVacuumLocalDirReclaimsDeletedSpace drives decode -> compact -> re-encode ->
// verify and confirms the shard set shrank and only live needles survive.
func TestVacuumLocalDirReclaimsDeletedSpace(t *testing.T) {
	dir := t.TempDir()
	opts := buildEncodedFixture(t, dir, "c1", 1, 20, 10)
...
// TestVacuumLocalDirLeavesNoScratchProbe is the regression test for the
// scratch-dir prober leak. VacuumLocalDir runs on the caller's scratch copy,
// which run.go deletes the moment the vacuum finishes; the compaction location
// must therefore be built with storage.NewScratchDiskLocation, which starts no
// statfs prober and writes no vol_dir.uuid. Using the volume server's
// NewDiskLocation here would leave a goroutine probing a removed directory
// forever and pin volumeServer_disk_error_status{<dir>,type="error"} = 1 on the
// worker, alongside four volumeServer_resource children.
//
// The prober probes once immediately, before any 60s tick, so a regression is
// visible without waiting.
func TestVacuumLocalDirLeavesNoScratchProbe(t *testing.T) {
	dir := t.TempDir()
	...
	}, 2*time.Second, 50*time.Millisecond, "scratch dir was probed")
	...
	// itself. A leaked prober is still alive here: it loops for 60s without a Close.
	var leaked int
	...
	require.Zero(t, leaked, "scratch dir left %d goroutine(s) behind (leaked disk prober)", leaked)
